...
export const deepClean = async (input, redactor, visited) => {
      ...
      if (_.isArray(value)) {
        ...
        for (let k = 0; k < value.length; k++) {
          ...
          if (isObjectNotFunction(item) || _.isArray(item)) {
            array[k] = await deepClean(item, redactor, visited);
          } else {
        ...
        output[key] = await deepClean(value, redactor, visited);
      } else {
