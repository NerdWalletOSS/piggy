...
import { EVENT_TYPE, EVENT_LEVEL, COLOR_HINT, FEATURE } from './constants';
...
const addXhrHookIfEnabled = async () => {
  ...
  XHR.prototype.send = async function send(body) {
    /* if there's a request interceptor installed, call through to it. this
    allows the calling application to re-write outgoing http requests if
    it needs to for debugging/diagnostic purposes */
    if (interceptRequest) {
      const args = this.__openArguments;
      ...
      const result = await interceptRequest(params);
      if (result !== false) {
        params = result || params;
        this.__openArguments = [
        ...
        this.__requestHeaders = { ...(params.headers || {}) };
      }
    }
    ...
    const timelinePriority = _.isNumber(config.timelinePriority)
      ? config.timelinePriority
      : TIMELINE_PRIORITY.HTTP;
    const hostname = getHostname(url || '');
    const stopwatchName = hostnameToAlias[hostname] || DEFAULT_STOPWATCH_NAME;
    const work = timeline
      .stopwatch(stopwatchName, timelinePriority, { colorHint })
      .start(url);
...
const setRequestInterceptor = (requestInterceptor) => {
  interceptRequest = requestInterceptor;
};

export default {
  setHostnameToAliasMap,
  setConfiguration,
